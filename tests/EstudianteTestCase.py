...
class EstudianteTestCase(unittest.TestCase):
    ...
    def test_agregar_estudiante_repetido ( self ) :
        resultado = self.sorteo.agregar_estudiante(apellidoPaterno="Ramos",apellidoMaterno = "Ortega",nombres = "Juan Carlos",elegible = True)
        self.assertNotEqual(resultado, True)

    def test_verificar_almacenamiento_agregar_estudiante ( self ):
        self.session=Session()
        estudiante=self.session.query(Estudiante).filter((Estudiante.apellidoPaterno == "Ramos") and (Estudiante.apellidoMaterno == "Ortega") and (Estudiante.nombres == "Juan Carlos")).first()
        self.assertEqual("Ramos Ortega Juan Carlos", estudiante.apellidoPaterno+" "+estudiante.apellidoMaterno+" "+estudiante.nombres)
